...
def get_judge_top4_from_1_csv():
    # Читаем CSV файл
    with open('1.csv', 'r', encoding='utf-8') as file:
        reader = csv.reader(file, delimiter=';')
        rows = list(reader)
    
    # Получаем заголовки (имена судей) - колонки 1-57 (индексы 0-56)
    judges = rows[0][1:]  # Пропускаем первую пустую колонку
    
    # Создаем словарь для хранения топ-4 каждого судьи
    judge_top4 = {}
    
    # Обрабатываем каждую строку с песнями (начиная с индекса 1)
    for row_idx, row in enumerate(rows[1:], start=1):  # id песен начинаются с 1
        song_id = row_idx
        
        # Обрабатываем оценки судей (колонки 1-57)
        for judge_idx, judge_name in enumerate(judges):
            if judge_idx < len(row) - 1:  # Проверяем, что колонка существует
                score_str = row[judge_idx + 1]  # +1 потому что первая колонка пустая
...
def update_html_with_correct_data():
    # Получаем топ-4 для каждого судьи
    judge_top4 = get_judge_top4_from_1_csv()
    ...
    judge_mapping = {
        ...
        " Отряд Котовскага": "Отряд Котовскага",  # Обратите внимание на пробел в начале
    ...
    participants_data = [
        ...
        { "id": 2, "name": "Отряд Котовскага", "song": "Плюшевые поэты - Отряд Котовскага", "score": 2.5, "isJudge": True, "top": judge_top4.get(" Отряд Котовскага", []) },
