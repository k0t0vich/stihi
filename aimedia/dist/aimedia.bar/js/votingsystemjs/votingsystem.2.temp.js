...
class VotingSystem {
    constructor(player, containerSelector, { user = null, eventUid = null, tourUid = null, voitedCount = 10 } = {}) {
        ...
        this.tracks = []; // Array of track objects: { element, uid, voted, listenProgress, ... }
    ...
    _bindVoteButtonClicks() {
        ...
        $('body').on('click', `${this.containerSelector} .vote-button`, function(e) {
            ...
            // Всегда открываем модалку выбора места
    ...
    openVoteModal($card, trackUid) {
        ...
        $modal.find('[data-place]').on('click', function() {
            const targetPlace = parseInt($(this).data('place'), 10);
            ...
            self._moveTrackToPosition(trackUid, targetPlace);

            // Обновляем рейтинги и бейджи
            self._updateRankings();
            self._checkAndRenderResultsButton();
            $modal.remove();
        });
    ...
    _updateTrackCard($card, trackUid) {
        ...
        const place = currentOrder.indexOf(trackUid) + 1; // 1-based

        if (track && track.voted) {
            $voteButton.text(`${place} место`).addClass('voted');
        } else {
            $voteButton.text('Голосовать').removeClass('voted');
        }
    }

    // Обновление всех карточек треков
    _updateAllTrackCards() {
        const $container = $(this.containerSelector);
        const self = this;
    ...
    // Сдвиг занятых мест вниз при вставке на занятое место
    _shiftPlacesDown(targetPlace, insertingTrackUid) {
        // Находим первое свободное место после targetPlace
        let freePlace = targetPlace + 1;
        while (freePlace <= this.MAX_VOTES && !this.remainingPlaces.includes(freePlace)) {
            freePlace++;
        }

        // Если не нашли свободное место в пределах MAX_VOTES - не сдвигаем
        if (freePlace > this.MAX_VOTES) {
            // Просто освобождаем трек на targetPlace
            const occupiedTrackUid = Object.keys(this.votes).find(uid => this.votes[uid] === targetPlace);
            if (occupiedTrackUid) {
                delete this.votes[occupiedTrackUid];
                this.remainingPlaces.push(targetPlace);
                this.remainingPlaces.sort((a, b) => a - b);
            }
            return;
        }

        // Сдвигаем треки вниз от freePlace к targetPlace
        for (let place = freePlace; place > targetPlace; place--) {
            const trackUidAtPrevPlace = Object.keys(this.votes).find(uid => this.votes[uid] === place - 1);

            if (trackUidAtPrevPlace && trackUidAtPrevPlace !== insertingTrackUid) {
                // Сдвигаем трек на следующее место
                this.votes[trackUidAtPrevPlace] = place;
            }
        }

        // Освобождаем targetPlace
        this.remainingPlaces = this.remainingPlaces.filter(p => p !== freePlace);
        this.remainingPlaces.push(targetPlace);
        this.remainingPlaces.sort((a, b) => a - b);
    }

    // Перемещение трека на нужную позицию в DOM (только для старого режима)
    _moveTrackToPosition(trackUid, targetPlace) {
        const $container = $(this.containerSelector);
        ...
        // Создаем список неотмеченных треков в текущем порядке
        const unvotedCards = currentOrder.filter(card => !this.votes[card.uid]);

        // Создаем map отмеченных позиций: { position: uid }
        const votedPositions = { ...this.votes };

        // Строим финальный массив
        const finalOrder = [];
        let unvotedIndex = 0;

        // Проходим по всем позициям (от 1 до общего количества карточек)
        for (let position = 1; position <= currentOrder.length; position++) {
            if (votedPositions[position]) {
                // На этой позиции должен быть отмеченный трек
                const uid = votedPositions[position];
                const found = currentOrder.find(card => card.uid === uid);
                if (found) {
                    finalOrder.push(found.element);
                }
            } else {
                // На этой позиции должен быть следующий неотмеченный трек (из очереди)
                if (unvotedIndex < unvotedCards.length) {
                    finalOrder.push(unvotedCards[unvotedIndex].element);
                    unvotedIndex++;
                }
            }
        }
        ...
        $container.empty().append(finalOrder);
    ...
    _init() {
        ...
        // Только для десктопа - drag and drop
        if (!detectIsMobile()) {
        ...
        this._renderVoteButtonsNew(); // Добавляем кнопки выбора места (НОВЫЙ РЕЖИМ)
        this._bindVoteButtonClicksNew(); // Привязываем обработчики кликов (НОВЫЙ РЕЖИМ)
    ...
    _injectModeToggleStyles() {
        ...
        $('head').append(`
            <style id="mode-toggle-styles">
                ...
                /* Стили для модалки выбора места */
    ...
    _updateRankings() {
        ...
        $cards.each(function() {
            ...
            // Обновляем кнопку голосования (вместо удаления)
            self._updateTrackCard($card, trackUid);
